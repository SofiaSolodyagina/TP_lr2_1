...
public:
	...
	void load() {
		...
		try
		{
			...
			if (numberOfLines <= 0 || numberOfLines % strNums != 0) throw std::runtime_error("Файл повреждён!");
	...
	void printHead() {
		cout << "\033[44;30m" << setw(36) << "ФИО" << setw(30) << "Знак зодиака" << setw(16) << "День рождения" << "\033[0m" << endl;
	}
